...
fetch(`/${corpus.id}/search/dictionary`)
...
clickZone.forEach(selectedZone => {
    selectedZone.addEventListener("click", () => {
        ...
        window.location.href = `/${corpus.id}/dictionary/${encodeURIComponent(letter)}`;
    });
});
...
function select(li) {
  ...
  resultsEl.style.display = 'none';
  activeIndex = -1;
  window.location.href = `/${corpus.id}/dictionary/word?id=${encodeURIComponent(id)}`;
}
